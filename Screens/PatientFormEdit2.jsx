...
export default function PatientFormEdit2(props) {
  ...
  const saveFormData = async data => {
    try {
      await AsyncStorage.setItem('patientForm2', JSON.stringify(data));
    } catch (e) {
  ...
  const loadFormData = async () => {
    try {
      const jsonData = await AsyncStorage.getItem('patientForm2');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (savedData) {
        // Use saved data only if no data from server or if needed for saving user edits
        if (!patientForm) {
   ...
   if (
    ...
    try {
      const response = await axios.put(
        `http://10.0.2.2:5000/updatepatientform/${id}`,
        formdata1,
      );
      if (response.data.status === 'ok') {
        // navigation.getParent()?.setOptions({
        ...
        Toast.show({
          type: 'success',
          text1: 'แก้ไขสำเร็จ',
          text2: 'แก้ไขข้อมูลบันทึกอาการแล้ว',
        });
