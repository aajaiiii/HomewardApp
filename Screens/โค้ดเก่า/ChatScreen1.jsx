...
function ChatScreen1({setUnreadCount}) {
  ...
  async function getData() {
    try {
      ...
      const response = await axios.post('http://192.168.0.57:5000/userdata', {
        token: token,
      });
  ...
  const fetchAllUsers = async userId => {
    try {
      const response = await fetch(
        `http://192.168.0.57:5000/allMpersonnelchat1?userId=${userId}`, 
      );
      ...
      const usersWithLastMessage = await Promise.all(
        data.data.map(async user => {
          const lastMessageResponse = await fetch(
            `http://192.168.0.57:5000/lastmessage/${user._id}?loginUserId=${userId}`,
          );
